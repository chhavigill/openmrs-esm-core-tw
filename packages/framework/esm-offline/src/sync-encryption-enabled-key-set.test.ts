...
const decryptedSyncItem = { x: 'y' };
const encryptedSyncItem = { encryptedData: 'someEncryptedData' };

jest.mock("@openmrs/esm-offline/src/encryption", () => ({
  encryption: true,
  decrypt: jest.fn(async (json) => decryptedSyncItem),
  encrypt: jest.fn(async (json) => encryptedSyncItem)
}))
...
describe("Sync Queue", () => {
  ...
  it("enqueues sync item with expected attributes", async () => {
    ...
    expect(queuedItems[0].content).toStrictEqual(encryptedSyncItem);
...
describe("getSynchronizationItems", () => {
  it("returns `content` of corresponding `getFullSynchronizationItems` call", async () => {
    ...
    expect(items[0]).toStrictEqual(decryptedSyncItem);
  });
});
